...
| `baseStyle` | String | optional | style to use for the base map. Possible values:<br>`OSlight`: OS Light style. <br>`OSoutdoor`: OS Outdoor style. <br>`OSroad`: OS Road style.<br>`streets`: Mapbox street is the closest style to Google. Amended to remove commercial points of interest at small scale.<br>`light`: Mapbox light grey style.<br>`dark`: Mapbox black style
<br>any other value, empty or omitted: the classical beige style we used for the initial leaflet maps |
