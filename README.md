...
| `showAddressSearch` | Object | optional | A collapsible search bar to be displayed at the top of the map to navigate to an address. Uses Hackney addresses API though a proxy. Can search for postcodes and free-text address elements (e.g. Maurice bishop, 210 Mare Street, E8 1HH.) You can set this to `true` to use the defaults. Or, to customise the tool, use the following sub-elements:<br>`addressSearchLabel`: The text above the search bar. Default is "Go to an address"<br>`addressSearchClue`: The greyed text clue inside search bar. Default is "Enter a Hackney postcode or address"<br>`addressSearchExpanded`: Expand or collapse the search. Must be set to open or closed. Default is open|
